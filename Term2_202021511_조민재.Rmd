...
시간 관계상 월요일, 화요일 연재 웹툰만 사용함.
```{r}
evaluate_df <- read.csv("evaluate_df.csv")

left_join(evaluate_df, info_df)
...
g <- ggplot(evaluate_df, aes(x=episode, y=starpoint, color=id))
g <- g+geom_line()
g
...
## 느낀점
데이터가 많고 클수록 무조건 좋은 것은 아니란 걸 알게되었다. 
모든 만화의 매 회차마다의 별점과 베스트 댓글 등을 수집하려고 했는데,
